...
void load(FILE* fp)
{
	...
	while (1)
	{
		if (read_line(fp, buffer, BUFF_LENGTH) <= 0)
			break;
		name = strtok(buffer, "#");	// 공백문자 전까지 tokenize하여 저장함
		if (strcmp(name, " ") == 0) // 이름이 존재하지 않을 경우 NULL리턴
			name = NULL;
		else
			name = strdup(name); // 이름을 복제하여 저장함
		title = strtok(NULL, "#");	// 공백문자 전까지 tokenize하여 저장함
...
Artist* add_artist(char* name)
{
	...
	while (p != NULL && strcmp(p->name, name) < 0)	// ordered list에서 p보다 작은 값이 나타날 때까지 반복
	{
		q = p;
		p = p->next;
	}
...
void status_chart()
{
	for (int i = 0; i < NUM_CHARS; i++)
	{
		...
		while (p != NULL)
		{
			creat_chart(p); //"A"가수의 리스트에 대해 함수 호출
			p = p->next;    // 다음 이름으로 이동
		}
	}
}
...
void creat_chart(Artist* p)
{
	...
	while (ptr_snode != NULL)
	{
		if (chart_index == 0) { // 차트가 비어있는 상황, play_chart[0]위치에 먼저 데이터 하나 저장
